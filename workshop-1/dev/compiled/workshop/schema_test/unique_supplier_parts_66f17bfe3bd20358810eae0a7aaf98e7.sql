...
from (
    ...
    select
        concat(part_key, supplier_key, supplier_name)

    from __dbt__CTE__supplier_parts
    where concat(part_key, supplier_key, supplier_name) is not null
    group by concat(part_key, supplier_key, supplier_name)
    having count(*) > 1
